...
withdrawal rights through official 1inch Aqua and Harbor's SwapVM-derived router.
...
| `HarborExecutor` | Authenticate the trader, collect computed input, invoke the router, pay fees/output and reject residue. |
| `HarborSwapVMRouter` | Pinned official Aqua settlement plus local `HarborPricing` and `HarborClaimGuard` instructions. No upstream files are modified. |
| `LidoAdapter` | Approved wstETH requests and measured issuer recovery to the fixed vault. |
| `LidoClaimFactory` / `LidoClaimReceipt` | Canonical indivisible withdrawal receipts, custody and final-holder recovery. |
| `LidoValuation` | Protocol-native conversions/finalized amounts plus separately authorized, expiring public NAV haircuts. |
...
trade. Anyone can read `Executor.quote(Trade)`; the trader calls
`Executor.execute(Trade)`. Execution obtains the canonical published order and
recomputes through SwapVM under the shared Book/Vault lock.

```text
Publisher -> Book.publishPricing(route, parameters)
Trader -> Executor.quote(intent) -> Book/kernel + SwapVM read simulation
Trader -> Executor.execute(intent)
       -> Book/Vault lock -> SwapVM pricing -> Aqua transfers / Book hooks
       -> fee + customer payout -> measured cash accounting -> unlock
```
...
Monetary units are WETH wei; factors use 1e18; fees use basis points; times use Unix
...
Curve capacity is fixed at deployment between 1 and 1e9 WETH FACE; target is at
most 90%, kappa at most 1%, discount between 50–100%, margins at most 5%, and each
operation cost at most 1 WETH. Inventory conversion ratios are restricted to
0.5–4 for the approved 18-decimal domain. These are numerical safety bounds,
...
A pending unstETH NFT owner approves an admitted factory and calls `wrap(id)`.
The factory escrows the NFT in a fixed-implementation clone and issues one
zero-decimal ERC-20 unit. It is a whole right, not an LP share. Governance separately
...
- FACE is derived from verified live inventory conversions and remaining native/
  receipt rights. Request/export moves the exposure rather than releasing it.
...
- Cash inside a recovered receipt is not vault trading cash; its FACE allocation
...
- Vault NAV uses independent public marks, not the bid. Price publication never
  ...
  checkpoint automatically before opening its lock.
...
`LidoValuation` reads underlying share totals, not a rounded one-token exchange
...
At most two native routes and 64 active native/receipt rights are traversed.
...
| `test/invariant/` | 1 | Partial recovery preserves outstanding acquisition basis until closure. |
| `test/gas/` | 1 | Runtime-size gate. |
| `test/fork/` | 4 | Pinned real issuer/token interactions. |

Default fuzzing uses 64 cases; the library-level partial-claim invariant runs
32 sequences of 16 calls. It is not a randomized full-vault campaign or a claim
that Lido supports partial payouts. `IssuerRecoveryTest` uses the actual native
...
modify issuer storage or inject issuer recovery cash. Canonical receipts cannot
import finalized requests. Written/compiled fork checks require an actual
...
Book is 24,220 bytes (356 bytes headroom), Vault 18,347, Executor 6,391,
router 23,017, adapter 6,421 and native valuation 6,185. Check all linked runtime sizes in `forge build --sizes`;
...
benchmarks. Maximum-portfolio gas and independent security review remain gaps.
...
addresses, links, code hashes and ABIs; capability getters are not attestations.
Configure each route's pricing policy and publish live pricing/valuation parameters
through their authorized callers before inviting trades. The native valuation
provider must bind the exact approved wstETH/queue/adapter pair.

Old signed-fill APIs, receiver reports and programs are not supported. No proxy
