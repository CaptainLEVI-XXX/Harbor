...
library IssuerOperations {
  ...
  /// @notice Move verified inventory into one to eight issuer rights, retaining settlement-asset-denominated cost.
  ...
  function request(
    ...
    if (
      adapter.BOOK() != address(this) || adapter.VAULT() != vault || adapter.BASE() != r.base
        || adapter.ASSET() != cashAsset
  ...
  /// @notice Recover a strictly ordered batch into the fixed vault, recording measured settlement-asset raw units.
  ...
  function recover(
    ...
    for (uint256 i; i < ids.length; ++i) {
      ...
      uint256 beforeBalance = SafeTransfer.balanceOf(cashAsset, vault);
      (uint256 cash, uint256 remaining) = IHarborAdapter(adapter).claim(ids[i], hints[i]);
      if (SafeTransfer.balanceOf(cashAsset, vault) != beforeBalance + cash) revert SettlementMismatch();
