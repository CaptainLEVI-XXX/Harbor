...
The same issuance suite also reaches the numerical share limit through deposits,
then proves that exact minting can consume the last 639,935 share units for one
asset wei even when `maxDeposit` is zero. `maxMint` must apply its own rounding
limit, not inherit the deposit limit; the final mint exhausts both capacities.
...
Operator changes and ordinary share transfers use a narrow local context; economic
mutations remain coordinated. No reporting-only Book/Vault valuation counters or
portfolio-hash notification are required for that lock.
