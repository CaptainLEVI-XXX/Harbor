...
/** WETH volume: abbreviated above ten thousand, whole below it. */
function volume(wei: bigint | null): string {
  if (wei === null) return '—';
  const whole = Number(wei / 10n ** BigInt(ASSET_DECIMALS));
  return whole >= 10_000 ? `${(whole / 1000).toFixed(2)}k` : whole.toFixed(0);
}
  ...
  return (
    <div className="list well">
      ...
      <div className="stot cols">
        ...
        <span className="num sub">{formatWeiFixed(queueTotal, ASSET_DECIMALS, 1)}</span>
      </div>
    </div>
  );
}
